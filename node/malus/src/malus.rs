...
enum NemesisVariant {
	...
	PvfPrepareWorker(peer_cli::ValidationWorkerCommand),

	#[allow(missing_docs)]
	#[command(name = "execute-worker", hide = true)]
	PvfExecuteWorker(peer_cli::ValidationWorkerCommand),
}
	...
	fn launch(self) -> eyre::Result<()> {
		...
		match self.variant {
			NemesisVariant::BackGarbageCandidate(opts) => {
				...
				peer_cli::run_node(cli, BackGarbageCandidates { percentage }, finality_delay)?
			},
